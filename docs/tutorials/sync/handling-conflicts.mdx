...
When your client's `lastMutationAt` doesn't match the server's state, you'll receive a **409 Conflict** error. This guide explains when conflicts occur and how to resolve them.
...
sequenceDiagram
    ...
    Client->>Server: POST /v1/sync?lastMutationAt=T_server
...
This quickly retrieves the current `lastMutationAt` without fetching all mutations.
