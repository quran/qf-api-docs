...
We have created a complete [Example OAuth2 React Native client](https://github.com/quran/oauth2-react-native-client-example) that you can clone and run. That repository shows the direct public-client exchange variant. If your client has a `client_secret`, keep the in-app login pieces and move token exchange to your backend as shown above.
...
Access tokens expire after 1 hour. For confidential clients, refresh them through your backend:
