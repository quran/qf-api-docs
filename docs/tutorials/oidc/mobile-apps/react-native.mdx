...
Keep the hosted login screen and PKCE flow in the app, then send `code + code_verifier + redirect_uri` to your backend for token exchange and refresh. If Quran Foundation explicitly registered your client with `token_endpoint_auth_method=none`, you can also do the token exchange directly in the app.
:::
