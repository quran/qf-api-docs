...
# Quran.com’s OAuth2 Example Web Integration
...
- Paste the following into `.env`:
  ...
  - For production use, please make sure to obtain your own client id and secret use by contacting [us](mailto:developers@quran.com).
...
## Accessing resources from QuranFoundation’s  Resources Server

- You can explore the list of all available APIs [here](/docs/category/auth-api).
- For example, to add a collection to your profile, you can use our interactive [collection API](/docs/auth_versioned/add-collection)
...
- Now, to make sure it was added, you can make a call to [get all collections API](/docs/auth_versioned/get-all-collections):
  - Put the `access_token` value in `o_t_h` header.
...
| `/callback` | Receives the callback from QuranFoundation’s authorization server |
...
- `scope` value will come from `.env` file. You can read more about all the available scopes [here](/docs/auth_versioned/scopes).
