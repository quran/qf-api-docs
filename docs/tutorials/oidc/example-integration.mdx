...
- Paste the following into `.env`:
  ...
  SCOPES = 'openid offline_access collection bookmark reading_session preference user'
  ```

:::info
The credentials shown here (`quran-demo` / `secret`) are **dummy placeholders** used only to demonstrate the configuration format.

They are **not linked to any real OAuth client**, so the authorization flow will **not complete**, and you will **not** be redirected to your callback URL.

To run the example end-to-end, you must submit your own OAuth application and provide:

1. A valid `CLIENT_ID` and `CLIENT_SECRET`, and  
2. Your callback URL (e.g., `http://localhost:3000/callback`) **exactly** in the [API Access Request form](request-access).

Only callback URLs registered through the form are accepted, and they must match exactly.
:::
