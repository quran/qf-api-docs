...
- Clients issued through [Request Access](/request-access) are currently provisioned as confidential clients unless Quran Foundation explicitly registers `token_endpoint_auth_method=none` for you.
...
If you are integrating during the hackathon or using a newly issued Request Access client, start with the confidential server-side example below or add a small backend/proxy for your app. The direct mobile examples only work if Quran Foundation explicitly provisioned your client as public.
:::

### React Native (Expo, only for explicitly public clients)

Use this only if Quran Foundation explicitly provisioned your client with `token_endpoint_auth_method=none`.
...
👉 **[Full React Native Example Repo](https://github.com/quran/oauth2-react-native-client-example)** only for explicitly public clients
...
- If the issued client is confidential, stop and build a backend/proxy instead of doing direct token exchange in the app
...
This direct mobile flow assumes the client is registered as public with `token_endpoint_auth_method=none`. If your client is confidential, move token exchange to a backend and keep the secret there. For newly issued Request Access clients, assume confidential unless Quran Foundation explicitly told you otherwise.
...
If your client is public (`token_endpoint_auth_method=none`), do not add `client_secret`. Only clients registered as public can omit it. If the client came from Request Access, assume the confidential example above applies unless Quran Foundation explicitly confirmed a public client.
...
❌ **Assuming every client can omit `client_secret`** — Only clients registered with `token_endpoint_auth_method=none` can do that; Request Access clients are confidential unless QF explicitly says otherwise
