...
- Or switch all three to prelive:
...
- Implement logout that calls the backend logout endpoint: include refreshToken in the request when your backend expects the app to forward tokens for revocation, or omit refreshToken when your backend stores and revokes refresh tokens itself; in all cases, clear local state
