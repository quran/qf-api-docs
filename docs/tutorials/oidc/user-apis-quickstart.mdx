...
**Prerequisites:** An app and `client_id` from [Developer Console](https://dev-console.quran.foundation/projects), plus at least one registered redirect URI. Backend/server apps also receive a one-time `client_secret`; browser/mobile apps do not.
...
:::important Confidential backend/server integration
...
If you selected a browser/mobile app in Developer Console, the client is public: exchange the code in the app using PKCE, then refresh with `client_id` and no `client_secret`. See the [manual OAuth2 tutorial](/docs/tutorials/oidc/getting-started-with-oauth2#client-types) for that flow; the server SDK example below is for confidential backend/server apps.
:::
