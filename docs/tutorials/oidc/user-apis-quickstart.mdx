...
For JavaScript and TypeScript apps, use `@quranjs/api/public` in app, browser, or mobile-facing code and `@quranjs/api/server` on the backend. To start from a complete template instead of wiring that manually, use [Starter With NPX](/docs/tutorials/oidc/starter-with-npx).

## Use This Page When

- You are adding signed-in Quran.com user features to an existing app.
- You need the recommended architecture for bookmarks, collections, notes, reading progress, goals, or preferences.
- You want the shortest path to one successful User API call.

Use the [OAuth2 tutorial](/docs/tutorials/oidc/getting-started-with-oauth2) instead when you need to implement the manual token flow, compare public and confidential clients in detail, or debug authorization, token exchange, refresh, and OIDC validation.
...
For most Quran Foundation User APIs integrations, keep this shape:

- The app opens the Quran Foundation hosted login screen.
- The app generates PKCE, `state`, and `nonce`.
- The redirect URI receives an authorization `code`.
- The app sends `code + code_verifier + redirect_uri` to your backend.
- The backend exchanges and refreshes tokens for confidential clients.
- The backend or serverless proxy calls User APIs with `x-auth-token` and `x-client-id`.
...
For step-by-step PKCE, state, nonce, token exchange, and refresh examples, use the [OAuth2 tutorial](/docs/tutorials/oidc/getting-started-with-oauth2).
