...
- If Quran Foundation explicitly registered your client with `token_endpoint_auth_method=none`, you can also use a fully public in-app token exchange flow.
:::
...
If you are integrating during the hackathon or using a newly issued Request Access client, keep the login screen and PKCE flow in your app, then use a small backend/proxy for token exchange and refresh.
:::
...
If Quran Foundation explicitly registered your client as public with `token_endpoint_auth_method=none`, you can swap the backend call above for a direct `exchangeCodeAsync(...)` call in the app.
...
- Implement logout that calls the backend logout endpoint and clears local state
- Mention that direct in-app token exchange is only for clients explicitly registered as public with token_endpoint_auth_method=none
...
This is the recommended path for clients currently issued through [Request Access](/request-access). The example assumes the client is confidential and registered with the default Hydra mode `client_secret_basic`. Keep `CLIENT_SECRET` on the server; code exchange and refresh happen on the server.
...
OIDAuthorizationService.discoverConfiguration(forIssuer: issuer) { config, error in
    ...
    // For clients issued through Request Access, use AppAuth's manual
    // no-code-exchange mode so your backend can redeem the returned
    // authorization code with CLIENT_SECRET.
}
```

If Quran Foundation explicitly registered your client as public with `token_endpoint_auth_method=none`, AppAuth can also complete the token exchange on-device.
...
- For confidential clients, use AppAuth's manual interaction mode and send the returned code + PKCE verifier to your backend exchange endpoint
- Mention that on-device token exchange is only for clients explicitly registered as public with token_endpoint_auth_method=none
...
In your callback handler, send the returned authorization code and PKCE verifier to your backend exchange endpoint. If Quran Foundation explicitly registered your client as public with `token_endpoint_auth_method=none`, you can also let AppAuth-Android exchange the code on-device.
...
- Mention that direct on-device token exchange is only for clients explicitly registered as public with token_endpoint_auth_method=none
...
- Current Request Access clients should treat refresh as a server-side confidential flow unless Quran Foundation explicitly registered the client as public
:::
