...
**Prerequisites:** A `client_id` from the [Developer Console](https://dev-console.quran.foundation/projects) and at least one registered redirect URI.
...
Clients that have a `client_secret` are confidential clients. Direct browser calls from third-party origins are not the recommended web pattern and may be rejected by CORS or origin allowlist checks.
