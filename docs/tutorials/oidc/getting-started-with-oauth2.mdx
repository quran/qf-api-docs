...
For a production-style JavaScript starter with the SDK, OAuth2, reader, search, notes, and bookmarks already wired together, use the official scaffold:

```bash
npx @quranjs/create-app@latest my-quran-app --template next --package-manager npm --install --git --sdk-source npm --yes
```

See [Starter With NPX](/docs/tutorials/oidc/starter-with-npx) for the scaffold walkthrough, or continue below for the underlying OAuth2 flow.
