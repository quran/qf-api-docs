...
:::important Current Request Access Default
...
If you received a `client_secret`, keep it on the server and perform both the authorization code exchange and refresh-token exchange on the server. If you call `/oauth2/token` without client authentication for that client, Hydra returns `invalid_client`.
:::
...
> ⚠️ Current Request Access clients are confidential unless Quran Foundation explicitly provisions `token_endpoint_auth_method=none` for your client. Never embed `client_secret` in a browser or mobile app.
...
If you do not register `token_endpoint_auth_method=none`, Hydra treats the client as confidential by default. In the current Request Access flow, that means you should assume server-side token exchange unless Quran Foundation explicitly confirms a public client registration. Calling `/oauth2/token` without authenticating a confidential client will fail with `invalid_client`.
:::
...
- QF_CLIENT_SECRET (required for current Request Access clients; omit it only if Quran Foundation explicitly registered the client as public with token_endpoint_auth_method=none)
...
- Default to a confidential server-side integration unless Quran Foundation explicitly confirms `token_endpoint_auth_method=none` for the client.
...
> Tip: Always persist your `code_verifier` **before** redirecting to `/oauth2/auth`. For current Request Access clients, keep it server-side (session or secure httpOnly cookie) and redeem the `code` on the server. Public native/mobile clients should only keep it in app state/storage if Quran Foundation explicitly registered the client with `token_endpoint_auth_method=none`.
...
:::tip Start With The Confidential Example Unless QF Told You Otherwise
If your client came from [Request Access](/request-access), use the confidential server-side examples below unless Quran Foundation explicitly confirmed that your client was registered with `token_endpoint_auth_method=none`.
:::

:::warning Match The Exchange Method To The Registered Client
...
- Default to the confidential server-side exchange unless Quran Foundation explicitly confirms public registration.
