 ...
 */
function buildAuthorizationUrl({ redirectUri, scope = 'openid offline_access user collection' }) {
...
async function exchangeCodeForTokensConfidential({ code, redirectUri, codeVerifier }) {
  const { authBaseUrl, clientId, clientSecret } = getQfOAuthConfig();

  if (!clientSecret) {
    throw new Error('Client secret is required for confidential client token exchange');
  }
...
async function getCollections({ accessToken }) {
  ...
  const res = await axios.get(
    `${apiBaseUrl}/auth/v1/collections?first=1`,
...
async function refreshAccessToken({ refreshToken }) {
  ...
  const res = await axios.post(
    ...
    {
      headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
      ...(isConfidential ? { auth: { username: clientId, password: clientSecret } } : {})
    }
  );
