...
Contact [us](mailto:osama@quran.com) to obtain OAuth 2.0 credentials such as a client ID and client secret that are known to both Quran.Foundation and your application. The set of values varies based on what type of application you are building. For example, a JavaScript application does not require a secret, but a web server application does.
...
After an application obtains an access token, it sends the token to a Quran.Foundation API in HTTP o_t_h request header.
