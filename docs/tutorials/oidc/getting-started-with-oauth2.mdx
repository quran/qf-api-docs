...
Clients issued through [Request Access](/request-access) are currently provisioned as **confidential clients** unless Quran Foundation explicitly confirms the client was provisioned with `token_endpoint_auth_method=none`.
...
- QF_CLIENT_SECRET (required on the backend for current Request Access clients; omit it only if Quran Foundation provisioned your client as public with token_endpoint_auth_method=none)
...
> Tip: Always persist your `code_verifier` **before** redirecting to `/oauth2/auth`. For current Request Access clients, keep it either in app state until callback and send it to your backend exchange endpoint, or keep it in a server-side session if the backend initiates the login flow. Public native/mobile clients should only redeem the code directly if Quran Foundation provisioned the client with `token_endpoint_auth_method=none`.
...
- Method: `POST`
- Public clients provisioned with `token_endpoint_auth_method=none`: send `client_id` in the body + `code_verifier`
- Confidential clients provisioned with `client_secret_basic` (default) or `client_secret_post`: authenticate the client with its secret
...
<details>
<summary><b>JavaScript (Node, axios — public PKCE client provisioned with `token_endpoint_auth_method=none`)</b></summary>
...
<details>
<summary><b>Python (requests — public PKCE client provisioned with `token_endpoint_auth_method=none`)</b></summary>
...
- Public clients provisioned with token_endpoint_auth_method=none: include client_id in body
- Confidential server clients provisioned with client_secret_basic (default) or client_secret_post: use client authentication

Implementation requirements
- Use authBaseUrl from getQfOAuthConfig():
  POST {authBaseUrl}/oauth2/token
- Default to the frontend/native app + backend exchange pattern unless Quran Foundation explicitly confirms the client was provisioned as public.
...
The same provisioning rule applies during refresh:
- Public clients provisioned with `token_endpoint_auth_method=none` can omit `client_secret` and send `client_id` in the body.
- Confidential clients provisioned with `client_secret_basic` (default) or `client_secret_post` must refresh on the server and authenticate the client.

If your client came from [Request Access](/request-access), assume the refresh flow is confidential and server-side unless Quran Foundation explicitly confirmed the client was provisioned as public.
...
async function refreshAccessToken({ refreshToken }) {
  ...
  // provisioned the client with token_endpoint_auth_method=none.
...
def refresh_access_token(refresh_token: str):
    ...
    # Public clients only omit client_secret if Quran Foundation provisioned
    # the client with token_endpoint_auth_method=none.
...
- Public clients provisioned with token_endpoint_auth_method=none: include client_id in body
- Confidential clients provisioned with client_secret_basic (default) or client_secret_post: use client authentication
- Default to confidential/server-side refresh unless Quran Foundation explicitly confirmed the client was provisioned as public
...
- Provisioning mode: Unless Quran Foundation provisioned the client with `token_endpoint_auth_method=none`, Hydra treats it as confidential and requires client authentication at `/oauth2/token`. This is the current default for clients issued through Request Access.
...
| ------------------------- | ------------------------------------------ | ---------------------------------------------------- |
| `invalid_client`          | Wrong client auth method, using the public-client flow with a confidential client, missing secret for a confidential client, or wrong environment | Verify provisioning mode, `client_id/secret`, and environment. If the client came from Request Access, switch to the server-side exchange unless QF explicitly confirmed the client was provisioned as public |
...
Use `openid offline_access user collection` as your default scope set. The `openid` scope returns an ID token with user identity, `offline_access` provides a refresh token for long-lived sessions, `user` grants access to user profile data, and `collection` enables bookmark and collection management. Follow the principle of least privilege — only request `collection` when your app actually needs it.

### How long do Quran Foundation access tokens last?

Access tokens expire after **1 hour** (3,600 seconds). Use the `refresh_token` (granted via the `offline_access` scope) to obtain a new access token without requiring the user to log in again. See [Step 5: Refresh the Access Token](#-step-5-refresh-the-access-token-offline_access-scope) for implementation details.
...
**Public clients** (SPAs, mobile apps) are provisioned with `token_endpoint_auth_method=none` and rely solely on PKCE for security — they never use a `client_secret`. Your client is public **only if** Quran Foundation has explicitly confirmed it.
...
This usually means you're using some variant of the **public client** token exchange flow (sending `client_id` in the request body without a secret) when your client was actually provisioned as **confidential**. For confidential clients, you must authenticate using HTTP Basic Auth (`client_id:client_secret`) or `client_secret_post`. Check the [Troubleshooting table](#-common-issues--troubleshooting) for more details.
