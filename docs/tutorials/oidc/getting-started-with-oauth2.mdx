...
Quran Foundation **User APIs** use the OAuth 2.0 **Authorization Code** flow with **PKCE** for authenticating users and authorizing access to their data. Choose the matching app type in Developer Console: backend/server apps use a confidential client and perform code exchange and refresh on a server, while browser/mobile apps use a public client and complete those steps with PKCE and no client secret.
...
Your app type in Developer Console determines the client type: **backend/server apps are confidential**, while **browser/mobile apps are public**.
...
This is the recommended pattern when a React Native, iOS, Android, or browser app is paired with a [confidential backend/server client](#client-types). If you selected a public browser/mobile app in Developer Console, exchange the code in the app with PKCE and no client secret instead.
...
| ------------------------- | ------------------------------------------ | ---------------------------------------------------- |
| `invalid_client`          | Wrong client type, using the public-client flow with a confidential client, missing secret for a confidential client, or wrong environment | Check the app type in Developer Console, use server-side client authentication for a backend/server app or PKCE without a secret for a browser/mobile app, and confirm the credentials and environment |
...
**Confidential clients** have a `client_secret` that is stored securely on a backend server and used during token exchange and refresh. Choose the **backend/server app** type in Developer Console for this model.

**Public clients** (SPAs, mobile apps) rely solely on PKCE for security and never use a `client_secret`. Choose the **browser/mobile app** type in Developer Console for this model.

Check the app type in Developer Console instead of guessing: backend/server apps are confidential, while browser/mobile apps are public. Then use the matching exchange pattern in [Step 3](#step-3-exchange-code-for-tokens).
