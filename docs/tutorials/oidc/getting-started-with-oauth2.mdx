...
For the current Request Access flow, treat your client as confidential unless Quran Foundation explicitly confirms it was provisioned with `token_endpoint_auth_method=none`.

That means:
- your app can handle the hosted login screen and PKCE
- your backend should exchange authorization codes and refresh tokens
- `client_secret` stays on the server only

If a confidential client calls `/oauth2/token` without client authentication, Hydra returns `invalid_client`.
:::
