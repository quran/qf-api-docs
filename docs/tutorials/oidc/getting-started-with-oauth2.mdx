...
If you do not register `token_endpoint_auth_method=none`, Hydra treats the client as confidential by default. Calling `/oauth2/token` without authenticating a confidential client will fail with `invalid_client`.
:::
...
- QF_CLIENT_SECRET (optional only when the OAuth client is registered as public with token_endpoint_auth_method=none; otherwise required for confidential server apps)
...
> Tip: Always persist your `code_verifier` **before** redirecting to `/oauth2/auth`. Public native/mobile clients can keep it in secure app state/storage; confidential web/server apps should keep it server-side (session or secure httpOnly cookie) and redeem the `code` on the server.
...
- Registration mode: Unless the client is explicitly registered with `token_endpoint_auth_method=none`, Hydra treats it as confidential and requires client authentication at `/oauth2/token`.
...
| ------------------------- | ------------------------------------------ | ---------------------------------------------------- |
| `invalid_client`          | Wrong client auth method, missing secret for a confidential client, or wrong environment | Verify registration mode, `client_id/secret`, and environment |
...
OAuth2 errors
- invalid_client: wrong client auth method, missing secret for a confidential client, wrong client_id/secret, or wrong environment
