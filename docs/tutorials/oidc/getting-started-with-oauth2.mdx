...
Use `openid offline_access` as your default scope set. The `openid` scope returns an ID token with user identity, and `offline_access` provides a refresh token for long-lived sessions. Follow the principle of least privilege — only request additional scopes for things like user profile, bookmarks, collections, and similar features when your app actually needs them.
