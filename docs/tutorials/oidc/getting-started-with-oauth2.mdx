...
If Quran Foundation does not provision your client with `token_endpoint_auth_method=none`, Hydra treats it as confidential by default. In the current Request Access flow, that usually means your app should collect the authorization code while your backend performs the token exchange. Calling `/oauth2/token` without authenticating a confidential client will fail with `invalid_client`.
:::
...
:::info Match The Exchange Method To The Provisioned Client Type
...
- Validate callback inputs:
  ...
  - server-initiated web flows: code_verifier must come from server storage
  - frontend/native app + backend exchange flows: accept code_verifier from the app callback payload and validate presence before forwarding it
- Never log:
  - client_secret
  - authorization code
  - code_verifier
...
- Frontend/native app + backend exchange flow can forward code + code_verifier safely.
