...
|---|---|---|
| **Provisioned with** | `client_secret_basic` (default) | `token_endpoint_auth_method=none` |
| **Has `client_secret`?** | ✅ Yes — keep it server-side only | ❌ No |
| **Token exchange** | Server-side with `client_secret` | In-app with PKCE only |
...
**Most [Request Access](/request-access) clients are confidential by default.** Your client is public **only if** Quran Foundation has explicitly confirmed `token_endpoint_auth_method=none`.
