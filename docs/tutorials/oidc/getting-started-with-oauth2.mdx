...
**Prerequisites:** An app and `client_id` from [Developer Console](https://dev-console.quran.foundation/projects). Your selected app type determines whether the client is confidential or public.
**Time:** ~30 minutes to implement.
**Result:** Your app can authenticate users and access their Quran.com synced data.
:::

Quran Foundation **User APIs** use the OAuth 2.0 **Authorization Code** flow with **PKCE** for authenticating users and authorizing access to their data. Choose the matching app type in Developer Console: a **Backend/server app** uses a confidential client and performs code exchange and refresh on a server, while a **Frontend or mobile app** uses a public client and completes those steps with PKCE and no client secret.
...
:::tip Prefer a generated Next.js app?
Use [Starter With NPX](/docs/tutorials/oidc/starter-with-npx) if you want OAuth2, sessions, reader/search, and signed-in user routes already wired. Continue here if you need to implement or debug the OAuth2/OIDC flow manually. The scaffold command is `npx @quranjs/create-app@latest my-quran-app --template next --package-manager npm --install --git --sdk-source npm --yes`.
:::
...
Your app type in Developer Console determines the client type: **Backend/server app** creates a confidential client, while **Frontend or mobile app** creates a public client.
...
> The diagram shows the confidential backend/server pattern. For a public browser/mobile app, the app exchanges the code directly with PKCE and never uses a `client_secret`.
...
- Create an app in Developer Console and follow the flow for its selected app type.
- For a backend/server app, keep QF_CLIENT_SECRET server-side. For a browser/mobile app, use PKCE without a client secret.
...
1. [Create an app in Developer Console](https://dev-console.quran.foundation/projects/new) to obtain your OAuth client credentials. Every app receives a `client_id`; confidential backend apps also receive a one-time `client_secret` for server-side use only.
2. Register one or more exact `redirect_uri` values in Developer Console. These must match exactly at runtime.

> **[Which client type do I have?](#client-types)** Backend/server apps are confidential — let the app handle login + PKCE, then exchange and refresh tokens on your backend. Browser/mobile apps are public and use PKCE without a secret. Never embed `client_secret` in a browser or mobile app.
...
- `ui_locales=LANGUAGE_TAGS` (optional OIDC preference for the hosted login and consent UI)
...
Use the exact scopes approved for your client. If Quran Foundation approved a parent scope such as
`note`, `post`, or `collection`, request that parent scope in the authorization URL. Do not also
request child scopes such as `note.read` or `post.create` unless those child scopes were separately
approved for the client. See [OAuth2 Scopes](/docs/user_related_apis_versioned/scopes#parent-and-child-scopes).

> Tip: Always persist your `code_verifier` **before** redirecting to `/oauth2/auth`. For confidential backend/server apps, keep it either in app state until callback and send it to your backend exchange endpoint, or keep it in a server-side session if the backend initiates the login flow. Public browser/mobile apps may redeem the code directly without a client secret.

For example, request `collection` only when the user actually creates or manages a collection.

### Localize the hosted authorization UI

Pass the optional OIDC `ui_locales` authorization-request parameter when the
user has a language preference for the hosted login and consent screens. It is
a space-separated, ordered list of BCP 47 language tags. It affects only the
hosted authorization UI; it does not change token claims, API responses, or
your application's language.

The hosted UI currently supports `en`, `ar`, `ur`, `id`, `tr`, `fr`, and `ru`.
The authorization server uses the first supported preference. Regional tags
fall back to their base language, so `fr-CA fr en` selects French. When
`ui_locales` has no supported preference, the server checks `Accept-Language`.
If neither source identifies a supported language, the hosted UI falls back to
English.
...
This is the recommended pattern when a React Native, iOS, Android, or browser app is paired with a [confidential backend/server client](#client-types). If you selected **Frontend or mobile app** in Developer Console, exchange the code in the app with PKCE and no client secret instead.
...
The same [client type rules](#client-types) apply during refresh. Backend/server apps are confidential and refresh on the backend with client authentication; browser/mobile apps are public and refresh with `client_id` and no client secret.
...
async function refreshAccessToken({ refreshToken }) {
  ...
  // Backend/server apps selected in Developer Console are confidential: use
  // HTTP Basic and keep refresh on the server. Browser/mobile apps are public:
  // include client_id and omit client_secret when refreshing.
...
def refresh_access_token(refresh_token: str):
    ...
    # Backend/server apps selected in Developer Console are confidential: use
    # HTTP Basic and keep refresh on the server. Browser/mobile apps are public:
    # omit client_secret and include client_id in the refresh request.
...
- App type: See [Confidential vs Public Clients](#client-types) and follow the flow that matches the app type selected in Developer Console.
...
| ------------------------- | ------------------------------------------ | ---------------------------------------------------- |
| `invalid_client`          | Wrong client type, using the public-client flow with a confidential client, missing secret for a confidential client, or wrong environment | Check the app type in Developer Console, use server-side client authentication for **Backend/server app** or PKCE without a secret for **Frontend or mobile app**, and confirm the credentials and environment |
...
| `invalid_scope`           | Scope misspelled, not approved for the client, or a child scope was requested when only the parent scope was approved | Request the exact approved scopes. If approved for `note`, request `note`, not `note.read` or `note.create` unless those child scopes were separately approved |
...
Request the exact scopes approved for your client. If your client is approved for a parent scope such as `note`, request `note`; do not request `note.read`, `note.create`, or other child scopes unless they were separately approved. See [OAuth2 Scopes](/docs/user_related_apis_versioned/scopes#parent-and-child-scopes).
...
**Confidential clients** have a `client_secret` that is stored securely on a backend server and used during token exchange and refresh. Choose **Backend/server app** in Developer Console for this model.

**Public clients** (SPAs, mobile apps) rely solely on PKCE for security and never use a `client_secret`. Choose **Frontend or mobile app** in Developer Console for this model.

Check the app type in Developer Console instead of guessing: **Backend/server app** creates a confidential client, while **Frontend or mobile app** creates a public client. Then use the matching exchange pattern in [Step 3](#step-3-exchange-code-for-tokens).
