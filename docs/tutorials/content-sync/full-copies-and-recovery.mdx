...
- Full copy of `word_by_word_translations:85` returns all current word translation rows for eligible resource content `85`, ordered by `word_id`, `priority`, and `id`.
...
Word-by-word translation records contain `id`, `resource_content_id`, `resource_id`, `word_id`, `language_id`, `language_name`, `text`, `priority`, and `updated_at`. Only approved, shareable, translation-only one-word resources have public snapshots; transliterations are unavailable and return `snapshot_not_found`.
