...
The UI reads from local storage. The sync engine updates that storage in the background. If connectivity to QF is unavailable, previously synced Content Sync content may remain available beyond seven days; sync promptly when connectivity returns.
...
| Network restored          | Run incremental sync promptly.                                 |
