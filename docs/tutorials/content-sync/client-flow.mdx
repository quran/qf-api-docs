...
- `resource_group`, such as `translations` or `word_by_word_translations`
- `resource_id`, such as translation resource content ID `19` or word-by-word translation resource content ID `85`

Row-level changes also include:

- `record_type`, such as `translation` or `word_translation`
...
The current sync contract accepts only `articles`, `recitations`, `tafsirs`, `translations`, and `word_by_word_translations` as resource groups. Word-by-word resources use `word_translation` row records.
