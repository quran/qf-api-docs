...
```mermaid
flowchart TD
    A["Choose resources filter"] --> B{"Has stored sync token for this exact filter?"}
...
Store one sync token per canonical resource filter. Do not reuse a token for a different filter.
...
| Store the token with the exact resource filter.  | Tokens are bound to the filter they were created for.               |
