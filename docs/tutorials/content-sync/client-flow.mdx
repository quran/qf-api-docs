...
- `resource_group`, such as `mushafs`, `translations`, or `chapter_recitations`
- `resource_id`, such as Mushaf ID `1`, translation resource content ID `19`, or `Audio::Recitation.id` `159` for a chapter recitation

Row-level changes also include:

- `record_type`, such as `translation`, `word_translation`, or `chapter_audio_file`
...
The current sync contract accepts only `articles`, `chapter_recitations`, `mushafs`, `recitations`, `tafsirs`, `translations`, and `word_by_word_translations` as resource groups.
