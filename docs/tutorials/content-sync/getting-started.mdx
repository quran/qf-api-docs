...
Instead of downloading every translation, word-by-word translation, tafsir, recitation, or article again, your app asks:
...
| Term            | Plain meaning                                                                                                |
| --------------- | ------------------------------------------------------------------------------------------------------------ |
| Resource        | One content item your app tracks, such as translation `19`, word-by-word translation `85`, tafsir `151`, recitation `10`, or article `123`. |
| Resource filter | The list of content your app wants to keep in sync, for example `translations:19;word_by_word_translations:85`.                                  |
...
Content Sync currently supports these resource groups: `translations`, `word_by_word_translations`, `tafsirs`, `recitations`, and `articles`.
...
For `word_by_word_translations`, use the resource content ID. Only approved, shareable, translation-only one-word resources are available. Transliteration resources are excluded.
...
sequenceDiagram
    ...
    App->>Sync: bootstrap=true for translation 19 and word-by-word translation 85
...
```bash
curl "https://apis.quran.foundation/content/api/v4/resources/sync?bootstrap=true&resources=translations:19;word_by_word_translations:85&per_page=100" \
...
For a word-by-word translation, use the `word_by_word_translations` group and its resource content ID:

```bash
curl "https://apis.quran.foundation/content/api/v4/resources/snapshots/word_by_word_translations/85" \
  -H "x-auth-token: $ACCESS_TOKEN" \
  -H "x-client-id: $CLIENT_ID"
```

Each record contains `id`, `resource_content_id`, `resource_id`, `word_id`, `language_id`, `language_name`, `text`, `priority`, and `updated_at`. Records are returned in canonical `word_id`, `priority`, and `id` order. Replace all local rows for `word_by_word_translations:85` with the returned `records` array.
...
```bash
curl "https://apis.quran.foundation/content/api/v4/resources/sync?sync_token=$SYNC_TOKEN&resources=translations:19;word_by_word_translations:85&per_page=100" \
  -H "x-auth-token: $ACCESS_TOKEN" \
  -H "x-client-id: $CLIENT_ID"
```
