...
Instead of downloading every Mushaf, translation, tafsir, recitation, or article again, your app asks:
...
Content Sync currently supports these resource groups: `mushafs`, `translations`, `word_by_word_translations`, `tafsirs`, `recitations`, and `articles`.

| Resource group                 | What the full copy contains                                                           | Changes included                                                                    |
| ------------------------------ | ------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------- |
| `mushafs`                      | Quran text, word positions, pages, layout data, and—where available—font asset metadata such as CDN URLs. Font files and page images are not included. | Mushaf, page, word, and—where available—font asset metadata changes.                  |
...
Mushaf Content Sync provides Quran text, word positions, pages, layout data, and font asset metadata such as CDN URLs. It does not include the font files themselves or page images. For rendering assets, see [Mushaf Fonts and Images](/legal/mushaf-fonts-and-images/) and the [Font Rendering guide](/docs/tutorials/fonts/font-rendering/).
...
```bash
curl "https://apis.quran.foundation/content/api/v4/resources/sync?bootstrap=true&resources=mushafs:1;translations:19;tafsirs:151&per_page=100" \
...
A full copy returns all current rows for one resource. For example, a Mushaf full copy returns its metadata, pages, font asset metadata, and words. Font files and page images are separate.
...
```bash
curl "https://apis.quran.foundation/content/api/v4/resources/sync?sync_token=$SYNC_TOKEN&resources=mushafs:1;translations:19;tafsirs:151&per_page=100" \
  -H "x-auth-token: $ACCESS_TOKEN" \
  -H "x-client-id: $CLIENT_ID"
```
