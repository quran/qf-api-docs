...
Instead of downloading every Mushaf, translation, tafsir, recitation, chapter recitation, or article again, your app asks:
...
| Term            | Plain meaning                                                                                                |
| --------------- | ------------------------------------------------------------------------------------------------------------ |
| Resource        | One content item your app tracks, such as translation `19`, tafsir `151`, ayah recitation `10`, chapter recitation `159`, or article `123`. |
...
Content Sync currently supports these resource groups: `mushafs`, `translations`, `word_by_word_translations`, `tafsirs`, `recitations`, `chapter_recitations`, and `articles`.
...
```bash
curl "https://apis.quran.foundation/content/api/v4/resources/sync?bootstrap=true&resources=chapter_recitations:159;mushafs:1;translations:19;tafsirs:151&per_page=100" \
...
```bash
curl "https://apis.quran.foundation/content/api/v4/resources/sync?sync_token=$SYNC_TOKEN&resources=chapter_recitations:159;mushafs:1;translations:19;tafsirs:151&per_page=100" \
  -H "x-auth-token: $ACCESS_TOKEN" \
  -H "x-client-id: $CLIENT_ID"
```
