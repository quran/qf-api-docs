...
Content Sync supports these resource groups after the Quran core rollout: `quran_core`, `mushafs`, `translations`, `word_by_word_translations`, `word_by_word_transliterations`, `tafsirs`, `recitations`, `chapter_recitations`, and `articles`. `quran_core:1` is not public until its content/licensing and attribution review is complete.
...
Once published, select `quran_core:1;mushafs:1` for canonical verse text and
...
The source/edition, redistribution terms, and attribution for canonical
`text_uthmani` require approval before `quran_core:1` can be published. SDK
support and this contract do not grant permission to redistribute it. A
snapshot may return `snapshot_not_found` until publication.
:::
...
After publication, fetch the canonical singleton separately:
