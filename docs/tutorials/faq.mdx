...
Content Sync does not support Quran text, but it supports `translations`, `word_by_word_translations`, `tafsirs`, `recitations`, and `articles`. Use the relevant [regular content endpoint](/docs/content_apis_versioned/4.0.0/content-apis/) for Quran text and other unsupported data. Unless QF expressly permits longer storage, the [Developer Terms' one-week storage limit](/legal/developer-terms) still applies to content that is not available through Content Sync.
