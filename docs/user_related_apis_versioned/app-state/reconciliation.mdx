...
A fresh account with no `syncToken` must bootstrap before the first pull: build the snapshot in
staging, drain changes from the terminal bootstrap token, and promote it atomically. Only then enter
the normal pull, captured replay, then pull-again loop. Queue writes may continue while network I/O
...
passes. Existing imports and low-level calls remain valid; reconciliation is opt-in. For runnable
imports, calls, pagination, conditional mutations, error handling, and account-bound transport
switching, use the dedicated [JavaScript App State SDK guide](/docs/sdk/javascript/app-state/) or
[Python App State SDK guide](/docs/sdk/python/app-state/).
