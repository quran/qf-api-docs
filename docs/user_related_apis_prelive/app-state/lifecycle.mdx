...
| Operation | Contract |
| --- | --- |
| `GET /auth/v1/users/connected-apps` | Groups every linked OAuth client under one logical app. Returns app identity/display metadata, environment, storage status/categories, recorded grant scopes, effective scopes, and approved last-access metadata. It never returns raw OAuth client or namespace identifiers or App State documents. |
| `POST /auth/v1/users/connected-apps/{appId}/lifecycle` | Accepts required `action` and high-entropy `requestId`, then returns `requestId`, `action`, overall `status`, `retryable`, and metadata-only `progress`. An exact retry reuses the same request ID and body. |
...
flowchart TD
    ...
    D --> P
    P -->|Partial| T[Retry the same logical request]
    P -->|Complete| F[Refresh Connected Apps inventory]
```
