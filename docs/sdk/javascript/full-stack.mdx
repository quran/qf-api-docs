...
description: "Recommended pattern for full-stack apps: the public SDK entrypoint in the frontend and server SDK entrypoint in the backend."
...
Do not use this when: You only have a backend or selected **Frontend or mobile app** for a public client with no backend exchange.
Backend required: Yes.
Allowed runtimes: Browser + backend, mobile + backend.
Required credentials: A **Backend/server app** from Developer Console, with its pre-live `client_id` in the frontend and its pre-live `client_id` plus one-time `client_secret` in the backend.
...
Both clients explicitly use pre-live so they match a newly created Console app. After production permissions are granted, switch the frontend and backend Client IDs, the backend secret, and both service URLs to production together.
...
5. Backend calls Content or approved User APIs as needed. Search calls require separate Search permission in Developer Console.
