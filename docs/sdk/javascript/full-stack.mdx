...
Required credentials: A **Backend/server app** from Developer Console, with its `client_id` in the frontend and its `client_id` plus one-time `client_secret` in the backend.
