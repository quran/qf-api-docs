...
- If you want **Content** or **Search**, use a backend and select **Backend/server app** in Developer Console.
- If you want **User APIs only** without a backend, select **Frontend or mobile app**; Developer Console provisions it as a public client.
- If you want **both** user features and Quran data, build a frontend plus backend app and select **Backend/server app**.
- Always follow the flow for the app type shown in Developer Console instead of guessing the client type.
...
flowchart TD
  ...
  D -->|Yes| F{"Which app type did you select in Developer Console?"}
  F -->|Frontend or mobile| G["SPA only or mobile only\nPublic client\nPKCE + user token"]
  F -->|Backend/server| H["Use frontend + backend\nConfidential client"]
```
...
| --- | --- | --- | --- | --- | --- | --- |
| SPA only | Public client | No for OAuth; browser-origin restrictions may still require an API proxy | Yes | No | No | `@quranjs/api/public` |
| Mobile only | Public client | No | Yes | No | No | `@quranjs/api/public` |
...
| Notes, bookmarks, or collections only | SPA or mobile app using the **Frontend or mobile app** type; add a proxy if browser-origin rules require one | These are User APIs and use a user token |
...
| Mobile app with signed-in user features only | Mobile app using the **Frontend or mobile app** type | Developer Console provisions a public client that uses PKCE without a secret |
...
Select **Frontend or mobile app** in Developer Console for this client type.
...
Select **Backend/server app** in Developer Console for this client type.
