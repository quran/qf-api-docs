...
Mushaf snapshots are typed with `MushafSnapshotRecord` and contain metadata,
page mappings, publicly distributable font assets, and positioned words:
...
const snapshot =
  await client.content.v4.resources.findSnapshot<MushafSnapshotRecord>(
    "mushafs",
    1,
  );
```
