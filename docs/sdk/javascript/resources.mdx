...
files and Mushaf images are not included. See the [Font Rendering guide](/docs/tutorials/fonts/font-rendering/)
for implementation details and the [Developer Terms](/legal/developer-terms/) for caching and bundling conditions.
...
When published, `quran_core:1` provides canonical Uthmani verse text, Surah
metadata, and Juz/Hizb/Rub-el-Hizb boundaries. The resource is separate from
Mushaf-specific pages and glyphs. The API may return `snapshot_not_found` until
the content/licensing and attribution review approves publication.
...
The SDK camel-cases `text_uthmani` and other response keys. Continue using
the same `resources` filter when polling, and replace all local core rows when
`RESOURCE_INVALIDATE` supplies a snapshot URL.
...
currently contains 77,429 records and includes row `id: 1` with normalized
`wordId: 1` and text `"bis'mi"`.
