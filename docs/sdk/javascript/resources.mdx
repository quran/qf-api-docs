...
Use content resource sync when a client needs to keep a local cache of public tafsirs, translations, recitations, or articles up to date without polling every resource row.
...
Snapshots return the current full resource payload. They are not sequence-bound, so later row mutations should still be handled idempotently.
## Languages
