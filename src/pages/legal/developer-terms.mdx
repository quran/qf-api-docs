...
A Developer may cache or bundle font files and Mushaf images served from QF’s documented CDN URLs for use in an Application if the Developer maintains an active account in the [Developer Console](https://dev-console.quran.foundation/projects) and credits Quran Foundation in a reasonably accessible place in the Application or its associated credits. The files may be distributed only as an integrated part of the Application. They may not be offered separately to others—for example, through the Developer’s own API, asset package, or standalone download.
