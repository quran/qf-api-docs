...
**Private backend versus redistribution.** Serving QF Content from a Developer-controlled backend within the Application's end-user experience is not, by itself, redistribution. Offering QF Content or raw API data to others as data remains subject to the separate commercial-license requirement above.

**Social-media videos are not redistribution.** Incorporating QF Content into a social-media video is an end-user use, not redistribution, provided that the Developer credits Quran Foundation in a reasonably accessible place, such as in the video itself or its caption, description, or credits. In these Terms, “redistribution” means offering QF Content or raw API data to others as data—for example, through the Developer’s own API, dataset, data feed, download, content package, or similar service.

**Audio.** Recitation metadata and audio URLs are distinct from the underlying recordings. Certain recordings may have their own rights holders and additional restrictions; access to their metadata or URLs does not change the rights applicable to the recordings.

**Derived Materials.** Derived Materials that include or permit reconstruction of QF Content remain subject to the same restrictions on that content.

Certain QF Content resources have their own underlying rights holders and may therefore carry additional restrictions. See [content rights by resource](/legal/resource-rights) for a summary.

### 2.3 Reservation of rights

Except for the licenses expressly granted, QF retains its rights in the APIs and QF Content it owns; other underlying rights holders retain their respective rights. Developer retains any rights in its original contributions to Derived Materials, subject to the rights in the underlying QF Content.
...
3. Cache or store QF Content longer than **1 week**, except where (a) QF has expressly permitted longer storage, or (b) the QF Content is available through the Content Sync APIs listed under [Content Available for Offline Sync](/docs/tutorials/content-sync/getting-started#content-available-for-offline-sync). If you rely on exception (b), you must [perform a next sync](/docs/tutorials/content-sync/getting-started#next-sync) at least every **7 days when connectivity to QF permits** and apply all available changes. This ongoing obligation applies to copies kept on a backend or an offline device. If connectivity to QF is unavailable for longer than seven days, the Application may continue using its previously synced Content Sync copy; perform a next sync promptly when connectivity returns and apply the available changes.
...
**Prepackaged content.** The Content Sync storage exception does not itself authorize distributing a prepackaged database or build-time bundle of QF Content. Section 2.2's redistribution terms apply, except for the font and Mushaf-image permission below.
...
**AI and retrieval use.** Retrieval-augmented generation (RAG) within the Application is distinct from using QF Content or User Data to build a machine-learning model under item 4. QF Content retained for retrieval, including content-derived embeddings that reproduce or permit reconstruction of it, remains subject to the applicable storage and use restrictions in these Terms.
